    ...
    def observe(self, x, t, y, i):
        ...
        if self.num_exemplars > 0:
            ...
            for tt in range(t):
                ...
                dl_loss += self.reg * self.kl(
                    self.lsm(self.net(inp_dist)
                             [:, offset1: offset2]),
                    self.sm(target_dist[:, offset1: offset2])) * self.nc_per_task
        if i % 100 == 99:    # every 100 mini-batches...

        # ...log the distillation loss
            writer.add_scalar('dl loss',
                            dl_loss,
                            i)
        ...
        if self.examples_seen == self.samples_per_task:
            ...
            self.num_exemplars = int(self.n_memories /
                                     (num_classes + len(self.mem_class_x.keys())))     # updatae the nunber of exemplars for each class
            offset1, offset2 = self.compute_offsets(t)
