...
def array_to_features_s(vx,mean,std,transform_nor):
	...
	for i in range(bsize):
		vx_i = vx[i]
		# vx_i = vx_i.reshape(3,32,32)
		vx_input = transform_nor(vx_i).reshape((1,3,32,32))
		# print("vx_input.shape++++++++++++++++++",vx_input.shape)
		# vx_i = np.transpose(vx_i, (1,2,0))
		# vx_i = Image.fromarray(np.uint8(vx_i*255),'RGB')
		# vx_input = preprocess(vx_i).unsqueeze(0).to(device)
		# image_features =  model_clip.encode_image(vx_input)
		all_features.append(torch.cat([vx_input]))
		# print("all_features.shape++++++++++++++++++",torch.cat(all_features).shape)
    # vx = vx.reshape(10,3,32,32)
    # print("vx.shape:",vx.shape)
    # # vx_out = np.transpose(vx, (0, 2, 3, 1))
    # features = model_clip.encode_image(vx.to(device))
...
def train(train_loader, nets, optimizer, criterions, epoch):
	...
	transform_nor = transforms.Normalize(mean=mean,std=std)
	...
	for i, (img, target, idx, sample_idx) in enumerate(train_loader, start=1):
		...
		t_img = array_to_features(img)

		# s_img = student_trans_F(img)
		# print("img_size++++++++++++",img.shape)
		s_img = array_to_features_s(img,mean,std,transform_nor)

		# print("s_img_size++++++++++++",s_img.shape)
		# print("t_img_size++++++++++++",t_img.shape)

		if args.cuda:
			t_img = t_img.cuda(non_blocking=True)
			s_img = s_img.cuda(non_blocking=True)
		...
		feat_t, out_t = tnet(t_img)
		_, _, _, _, feat_s, out_s = snet(s_img)
